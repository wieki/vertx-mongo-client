 ...
 */
public class JsonObjectCodec extends AbstractJsonCodec<JsonObject, JsonArray>
	...
	public JsonObject generateIdIfAbsentFromDocument(JsonObject json) {
		...
		if (!documentHasId(json)) {
			...
			JsonObject oid = new JsonObject().put(OID_FIELD, id.toHexString());
			
			json.put(ID_FIELD, oid);
		}
		return json;
	}
	...
	@Override
	protected void beforeFields(JsonObject object,
	...
	@Override
	protected void forEach(JsonObject object,
			BiConsumer<String, Object> objectConsumer) {
	...
	protected BsonType getBsonType(Object value) {
		...
		if (type == BsonType.DOCUMENT) {
			...
			// not supported yet
			/*
			 * else if (obj.containsKey("$binary")) { return BsonType.BINARY; }
			 * else if (obj.containsKey("$maxKey")) { return BsonType.MAX_KEY; }
			 * else if (obj.containsKey("$minKey")) { return BsonType.MIN_KEY; }
			 * else if (obj.containsKey("$regex")) { return
			 * BsonType.REGULAR_EXPRESSION; } else if
			 * (obj.containsKey("$symbol")) { return BsonType.SYMBOL; } else if
			 * (obj.containsKey("$timestamp")) { return BsonType.TIMESTAMP; }
			 * else if (obj.containsKey("$undefined")) { return
			 * BsonType.UNDEFINED; } else if (obj.containsKey("$numberLong")) {
			 * return BsonType.INT64; } else if (obj.containsKey("$code")) {
	...
	@Override
	protected void writeObjectId(BsonWriter writer, String name, Object value,
			EncoderContext ctx) {
	
		writer.writeObjectId(new ObjectId(((JsonObject) value).getString(OID_FIELD)));
	}
	...
	protected Object readDateTime(BsonReader reader, DecoderContext ctx) {
		...
		result.put(
				...
				OffsetDateTime.ofInstant(
						...
						ZoneOffset.UTC).format(ISO_OFFSET_DATE_TIME));
		return result;
	}

	@Override
	protected void writeDateTime(BsonWriter writer, String name, Object value,
			EncoderContext ctx) {
		writer.writeDateTime(OffsetDateTime
				.parse(((JsonObject) value).getString(DATE_FIELD)).toInstant()
				.toEpochMilli());
	}
}
